...
class User():
    ...
    class Appliance():
        ...
        def __init__(self, user, n=1, P=0, w=1, t=0, r_t=0, c=1, fixed='no', fixed_cycle=0, occasional_use=1, flat='no',
                     thermal_P_var=0, pref_index=0, wd_we_type=2, P_series=False):
        ...
        def windows(self, w1=np.array([0, 0]), w2=np.array([0, 0]), r_w=0, w3=np.array([0, 0])):
            ...
            self.daily_use[w1[0]:(w1[1])] = np.full(np.diff(w1),
                                                    0.001)  # fills the daily use profile with infinitesimal values that are just used to identify the functioning windows
            ...
            self.daily_use_masked = np.zeros_like(ma.masked_not_equal(self.daily_use,
                                                                      0.001))  # apply a python mask to the daily_use array to make only functioning windows 'visibile'
            self.random_var_1 = int(r_w * np.diff(
                w1))  # calculate the random variability of window1, i.e. the maximum range of time they can be enlarged or shortened
        ...
        def specific_cycle_1(self, P_11=0, t_11=0, P_12=0, t_12=0, r_c1=0):
            ...
            self.fixed_cycle1 = np.concatenate(
                ((np.ones(t_11) * P_11), (np.ones(t_12) * P_12)))  # create numpy array representing the duty cycle
