...
"""-------Appliances residential building 1 in Director's Housing area----------"""
...
"""-------Appliances Residential building 2 in Director's housing area----------"""
...
Decoder_SR = SR.Appliance(SR,1,7,1,8*60,0.1,5)
#time window: 10:00-23:59
Decoder_SR.windows([10*60,23*60+59],r_w=0.35)

Phone_SR = SR.Appliance(SR,4,7,1,2*60,0.2,5)
#time window: 07:00-19:00
Phone_SR.windows([7*60,19*60],r_w =0.35)

PC_SR = SR.Appliance(SR,4,60,1,2*60,0.1,5)
#time window: 07:00-19:00
...
Fan_SC = SC.Appliance(SC,3,50,3,10*60,0.2,5)
#time window 1: 12:00-14:00
#time window 2: 19:00-07:00
Fan_SC.windows([12*60,14*60],[19*60,23*60+59],0.35,[0,7*60])

Phone_SC = SC.Appliance(SC,3,7,1,2*60,0.2,5)
#time window: 07:00-23:59
Phone_SC.windows([7*60,23*60+59],r_w =0.35)

PC_SC = SC.Appliance(SC,3,60,1,2*60,0.1,5)
#time window: 07:00-23:59
PC_SC.windows([7*60,23*60+59],r_w = 0.35)

Iron_SC = SC.Appliance(SC,1,1000,1,1*60,0.1,5,occasional_use = 0.5)
#time window: 10:00-21:00
Iron_SC.windows([10*60,21*60],r_w = 0.35)

Indoor_LED_SC = SC.Appliance(SC,6,9,2,4*60,0.2,5)
#time window 1: 06:00-07:00
#time window 2: 19:00-23:59
