...
def dispatch_max_sc(pv, demand, param, return_series=False):
    ...
    bat_capacity = param['BatteryCapacity']
    bat_maxpower = param['MaxPower']
    ...
    #first timestep = 0
    '''---Batteriet starter på 50 %---'''
#    LevelOfCharge[0] = bat_capacity / 2  # DC
    
    '''---Batteriet starter på 75 %---'''
    LevelOfCharge[0] = bat_capacity * 0.75  # DC

    for i in range(1,Nsteps):
        #PV to storage
        if LevelOfCharge[i-1] >= bat_capacity:  # if battery is full
                pv2store[i] = 0
        else: #if battery is not full
            if LevelOfCharge[i-1] + res_pv[i] * timestep > bat_capacity:  # if battery will be full after putting excess
                pv2store[i] = min((bat_capacity - LevelOfCharge[i-1]) / timestep, bat_maxpower) / n_bat
            else:
                pv2store[i] = min(res_pv[i], bat_maxpower)

        #Storage to load
        store2inv[i] = min(bat_maxpower,  # DC
                           res_load[i] / n_inv,
    ...
    energySurplus = (res_pv - pv2store) * n_inv      # AC
    notCoveredDemand = demand - inv2load             # AC
    ...
    out = {'pv2inv': pv2inv,
            ...
            'pv2store': pv2store,
            'inv2load': inv2load,
            'notCoveredDemand': notCoveredDemand,
            'store2inv': store2inv,
...
def dispatch_max_sc_grid_pf(pv, demand, param_tech, return_series=False):
    ...
    """
    bat_capacity = param_tech['BatteryCapacity']
    bat_maxpower = param_tech['MaxPower']
    ...
    def find_threshold(pv_day_load, bat_size):
        """Find threshold of peak shaving (kW). The surplus electricity *(fed to the grid)* is capped by a specific threshold.
        ...
        bat_size: Battery size
        """

        def get_residual_peak(thres):
            shaved_peak = np.maximum(pv_day_load - thres, 0)
            return sum(shaved_peak) * param_tech['timestep'] - bat_size

        if sum(pv_day_load) * param_tech['timestep'] <= bat_size:  # if the battery can cover the whole day
            return 0
        else:
    ...
    '''---Batteriet er 50%---'''
#    LevelOfCharge[0] = bat_capacity / 2 # Initial storage is half full in this casw # DC
    
    '''---Batteriet starter på 75 %---'''
    LevelOfCharge[0] = bat_capacity * 0.75  # DC

    # For the residual pv find the threshold above which the energy should be stored (first day)
    threshold = find_threshold(res_pv_val[0: 0 + int(23 / timestep)], bat_capacity - LevelOfCharge[0])
    ...
    for i in range(1, Nsteps):  # Loop hours
        ...
        if i % int(24/timestep) == 0:
            threshold = find_threshold(res_pv_val[i:i + int(23 / timestep)],
                                       bat_capacity - LevelOfCharge[i])

        # Excess energy
        if res_pv[i] * n_inv < threshold:  # If residual load is below threshold
            energySurplus[i] = res_pv[i] * n_inv  # Sell to grid what is not consumed
        else:  # If load is above threshold
            energySurplus[i] = threshold * n_inv  # (Sell to grid what is below the threshold)
            pv2store[i] = min(max(0, (res_pv[i] - threshold) * n_bat / n_inv),
                              (bat_capacity - LevelOfCharge[i - 1]) / timestep )  # Store what is above the threshold and fits in battery
        pv2inv[i] = pv2inv[i] + energySurplus[i] / n_inv  # DC

        store2inv[i] = min(bat_maxpower,  # DC
                           res_load[i] / n_inv,
                           LevelOfCharge[i - 1] / timestep)

        LevelOfCharge[i] = min(LevelOfCharge[i - 1] - (store2inv[i] - pv2store[i]) * timestep,
                               bat_capacity ) # DC
